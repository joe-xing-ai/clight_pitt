...
def upload_videos_to_cloud(s3, data_path, bucket, text):

    video_entries = []
    for root, dirs, files in os.walk(data_path):
        for file in files:
            if text:
                if file.endswith(".txt") or file.endswith(".TXT") or file.endswith(".csv") or file.endswith(".CSV"):
    ...
    logging.info("upload_data: found %s files, getting ready to upload now..." % n_videos)
    ...
    for i in range(n_videos):
        ...
        logging.info("upload file %s out of total %s files: %s to %s ... " % (i, n_videos, file_path, cloud_key))
...
def main():
    ...
    parser.add_option('--text', action="store_true", default=False, help=None)
    ...
    upload_videos_to_cloud(s3_resource, options.data_path, options.bucket, options.text)
